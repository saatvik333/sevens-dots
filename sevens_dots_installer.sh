...
install_pacman_packages() {
  ...
  # Show live output with tee while also logging
  if sudo pacman -S --needed --noconfirm "${PACMAN_PACKAGES[@]}" 2>&1 | tee -a "$LOG_FILE" | \
     grep --line-buffered -E "^\(|installing|upgrading|warning|error"; then
...
install_aur_packages() {
  ...
  info "This may take several minutes (especially for packages that need compilation)..."
  echo ""
  
  # Show live output with tee while also logging, filtering for useful progress info
  if "$AUR_HELPER" -S --needed --noconfirm "${AUR_PACKAGES[@]}" 2>&1 | tee -a "$LOG_FILE" | \
     grep --line-buffered -E "^\(|installing|upgrading|Cloning|Building|warning|error|==>"; then
...
install_colloid_theme() {
  ...
  while [ "$attempt" -le "$max_attempts" ]; do
    info "Cloning Colloid theme repository (attempt $attempt/$max_attempts)..."
    ...
    # Cleanup and retry if not last attempt
    rm -rf "$theme_dir"
    if [ "$attempt" -lt "$max_attempts" ]; then
      warn "Installation failed (attempt $attempt/$max_attempts). Retrying..."
      theme_dir=$(mktemp -d)
      sleep 2
    fi
    ((attempt++))
  ...
  warn "Failed to install Colloid theme after $max_attempts attempts."
  return 1
}
...
install_rosepine_theme() {
  ...
  while [ "$attempt" -le "$max_attempts" ]; do
    info "Cloning Rose Pine theme repository (attempt $attempt/$max_attempts)..."
  ...
  warn "Failed to install Rose Pine theme after $max_attempts attempts."
  return 1
}
...
install_osaka_theme() {
  ...
  while [ "$attempt" -le "$max_attempts" ]; do
    info "Cloning Osaka theme repository (attempt $attempt/$max_attempts)..."
  ...
  warn "Failed to install Osaka theme after $max_attempts attempts."
  return 1
}
...
main() {
  ...
  step "Rust Toolchain Configuration"
  cargo_fix
  add_summary "Rust toolchain configured for AUR package builds"
